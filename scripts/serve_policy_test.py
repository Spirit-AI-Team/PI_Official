...
DEFAULT_CHECKPOINT: dict[EnvMode, Checkpoint] = {
    ...
    EnvMode.SPI0_ARX_FULL: Checkpoint(
        ...
        dir="/pfstem/likaiyu/resources/checkpoints/spi0_aloha_eef_full/spi0_aloha_eef_full_rot6d_0312/29999",
    ),
...
def main(args: Args) -> None:
    ...
    record_root_path = "/pfstem/wenxuan/code/tmp/mozbrain/policy_results/"
    step_list = glob.glob("/pfstem/wenxuan/code/tmp/mozbrain/policy_results/step_*.npy")
    total_num_steps = len(step_list)

    # states_list = []
    res = []
    for step_idx in range(total_num_steps):
        step_data_path = os.path.join(record_root_path, f"step_{step_idx}.npy")

        data = np.load(step_data_path, allow_pickle=True).item()
        data = flax.traverse_util.unflatten_dict(data, sep="/")
        obs = data['inputs']
        obs['images']['cam_high'] = (obs['images']['cam_high'] * 255).astype(np.uint8)
        obs['images']['cam_left_wrist'] = (obs['images']['cam_left_wrist'] * 255).astype(np.uint8)
        obs['images']['cam_right_wrist'] = (obs['images']['cam_right_wrist'] * 255).astype(np.uint8)
        # convert to 6, 1, 6, 1
        obs['state'] = obs['state'][[2, 3, 4, 5, 6, 7, 0, 8, 9, 10, 11, 12, 13, 1]]
        gt_actions = data['outputs']['actions'][:, [2, 3, 4, 5, 6, 7, 0, 8, 9, 10, 11, 12, 13, 1]]

        # states_list.append(obs['state'])
        import ipdb; ipdb.set_trace()
        results = policy.infer(obs)

        diff = jnp.square(gt_actions - results['actions'])
        res.append(diff[:, [0, 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12]])

    import ipdb; ipdb.set_trace()
    print (f'diff: {np.mean(np.array(res))}')
